...
fir_df = pd.DataFrame(mac_data)
mac_df = pd.DataFrame(fir_data)
tmp_df = pd.merge(fir_df, mac_df, on=['kernel_size', 'in_channels', 'out_channels', 'input_size'], suffixes=('_fir', '_mac'))
df = tmp_df.copy()
...
scatter = plt.scatter(df['input_size'], df['time'], c=df['out_channels'], cmap='cool', s=100)
